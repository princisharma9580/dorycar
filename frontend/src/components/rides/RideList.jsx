...
const RideList = ({ currentUser, searchResults }) => {
  ...
  const handleBookRide = async (rideId) => {
    ...
    if (!currentUser) {
      toast.warn("Please login to book a ride.");
      navigate("/login")
      return;
    }
    try {
  ...
  const handleViewMap = (origin, destination) => {
    if (!currentUser) {
      toast.warn("Please login to view map.");
      navigate("/login")
      return;
    }
  ...
  const handleShareRide = (ride) => {
    if (!currentUser) {
      toast.warn("Please login to share your ride.");
      navigate("/login")
      return;
    }
                              ...
                              <div className="text-center">
                                ...
                                <div className="flex items-center">
                                  ...
                                  <span className="text-xs">
                                    {ride.creator?.averageRating?.toFixed(1) ||
                                      "N/A"}
                                  </span>
                                </div>
                              </div>
